...
if(isset($_GET['deleteid'])) {
	if(is_numeric($_GET['deleteid'])) {
	   ...
	   $sql->removeQuery($querys->removeJob($jobid));
                ...
                $sql->insertQuery($querys->insertActivity("Deleted Job ID: $jobid", $today, $userDetails[uid]));
...
<html lang="en">
  ...
  <body>
      ...
      <div class="container">
        ...
        <div class="navbar-collapse collapse">
          <ul class="nav navbar-nav">
            <li class="active"><a href="joblist.php"><i class="fa fa-wrench"></i> Repairs</a></li>
    ...
    <div class="container">
         ...
         <input name="searchitem" type="text" id="searchitem" size="30" placeholder="Search for an open job" />
         ...
         <br />
         <span class="flagtxt">PINK = Job has been waiting initial check for 5+ Days.</span><br />
         <span class="flagtxt-info">ORANGE = Job status update requested.</span>
		 <br /><br />
...
if (isset($_POST['searchtype']) && 
	...
	$_POST['searchsubmit']) {
		...
		if($searchtype == NULL	|| $searchtype == " ") {
		  ...
		  if ($searchtype == 1) {
		    $jobs = $sql->fetchWhileRows($querys->searchJobSN($searchitem,"0"));
		  ...
		  if ($searchtype == 3) {
		    $jobs = $sql->fetchWhileRows($querys->searchJobSO($searchitem,"0"));
			echo "<strong>Search Results:</strong> <br /><br />";
		    // If we returned no results
		    if ($jobs == NULL) {
		    echo "No search results found. <br /><br />";
		  ...
		  if ($searchtype == 2) {
		    ...
		    $jobs = $sql->fetchWhileRows($querys->searchJobAccNo($cid['id'], 0));
		    echo "<strong>Search Results:</strong> <br /><br />";
		    // If we returned no results
		    if ($jobs == NULL) {
		    echo "No search results found. <br /><br />";
		  ...
		  if ($searchtype == 4) {
                    ...
                    $jobs = $sql->fetchWhileRows($querys->searchJobCustomerNameCID($cid['id'], 0));
		    echo "<strong>Search Results:</strong> <br /><br />";
		    // If we returned no results
		    if ($jobs == NULL) {
		    echo "No search results found. <br /><br />";
		  ...
		  if ($searchtype == 5) {
		    $jobs = $sql->fetchWhileRows($querys->searchJobEngineerName($searchitem,"0"));
			echo "<strong>Search Results:</strong> <br /><br />";
		    // If we returned no results
		    if ($jobs == NULL) {
		    echo "No search results found. <br /><br />";
...
$rows = $sql->runNumRowsQuery($querys->getJobList());
...
$jobs = $sql->fetchWhileRows($querys->getPaginationTableOpen('jobs', 'id', 'ASC', $limit));
...
if($last != 1){
	...
	if ($pagenum > 1) {
		$paginationCtrls .= '<li><a href="joblist.php?page=1"> First </a></li>';
        $previous = $pagenum - 1;
		$paginationCtrls .= '<li><a href="joblist.php?page='.$previous.'"> Previous </a></li>';
		// Render clickable number links that should appear on the left of the target page number
		for($ib = $pagenum-2; $ib < $pagenum; $ib++){
			if($ib > 0){
				$paginationCtrls .= '<li><a href="joblist.php?page='.$ib.'">'.$ib.'</a></li>';
			}
	    }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= '<li><a href="joblist.php?page='.$next.'"> Next </a></li>';
        $paginationCtrls .= '<li><a href="joblist.php?page='.$last.'"> Last ('.$last.') </a></li>';
    }
}
