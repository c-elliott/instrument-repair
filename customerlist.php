...
// Delete an existing customer if we recieve GET info
if(isset($_GET['deleteid'])) {
	if(is_numeric($_GET['deleteid'])) {
       $rawcustomerid = $_GET['deleteid'];
       $customerid = $global->Clean($rawcustomerid);	
	   $sql->removeQuery($querys->removeCustomer($customerid));
	// Success window
...
<html lang="en">
  <head>
    ...
    <title>Instrument Repair Portal : Customer List</title>
  ...
  <body>
      ...
      <div class="container">
        ...
        <div class="navbar-collapse collapse">
          <ul class="nav navbar-nav">
            ...
            <li class="active"><a href="customerlist.php"><i class="fa fa-users"></i> Customers</a></li>
    ...
    <div class="container">
         ...
         <input name="searchitem" type="text" id="searchitem" size="30" placeholder="Search for a customer" />
...
if (isset($_POST['searchtype']) && 
	...
	$_POST['searchsubmit']) {
		...
		if($searchtype == NULL	|| $searchtype == " ") {
		  ...
		  if ($searchtype == 1) {
		    $customers = $sql->fetchWhileRows($querys->searchCustomerAccNo($searchitem));
		  ...
		  if ($searchtype == 2) {
		    $customers = $sql->fetchWhileRows($querys->searchCustomerName($searchitem));
			echo "<strong>Search Results:</strong> <br /><br />";
		    // If we returned no results
		    if ($customers == NULL) {
		    echo "No search results found. <br /><br />";
...
$rows = $sql->runNumRowsQuery($querys->getCustomerList());
...
$customers = $sql->fetchWhileRows($querys->getPaginationTable('customers', 'id', 'ASC', $limit));
...
if($last != 1){
	...
	if ($pagenum > 1) {
		$paginationCtrls .= '<li><a href="customerlist.php?page=1"> First </a></li>';
        $previous = $pagenum - 1;
		$paginationCtrls .= '<li><a href="customerlist.php?page='.$previous.'"> Previous </a></li>';
		// Render clickable number links that should appear on the left of the target page number
		for($ib = $pagenum-2; $ib < $pagenum; $ib++){
			if($ib > 0){
				$paginationCtrls .= '<li><a href="customerlist.php?page='.$ib.'">'.$ib.'</a></li>';
			}
	    }
    }
    ...
    if ($pagenum != $last) {
        $next = $pagenum + 1;
        $paginationCtrls .= '<li><a href="customerlist.php?page='.$next.'"> Next </a></li>';
        $paginationCtrls .= '<li><a href="customerlist.php?page='.$last.'"> Last ('.$last.') </a></li>';
    }
}
...
echo "            <td>" . $customers[$i]['customername'] . "</td>";
echo "            <td>" . $customers[$i]['customerno'] . "</td>";
echo "            <td>" . $customers[$i]['customercontact'] . "</td>";
echo "            <td>" . $customers[$i]['customerphone'] . "</td>";
echo "            <td>" . $customers[$i]['customeremail'] . "</td>";

// USERLEVELS : Only available for userlevel 2 and above
if ($_SESSION['userlevel'] >= 2) {

echo "            <td><a href=\"JavaScript:newPopup('customer_manage.php?id=" . $customers[$i]['id'] . "');\" ><button type=\"button\" class=\"btn btn-primary btn-xs\">Details</button></a> <a href=\"customerlist.php?deleteid=" . $customers[$i]['id'] . "\" onclick=\"return confirm('Are you sure you want to delete Customer ID: " . $customers[$i]['id'] . "?')\"><button type=\"button\" class=\"btn btn-danger btn-xs\">Delete</button></a></td>";
echo "          </tr>";
// USERLEVELS : Allow userlevel 1 to see details
} elseif ($_SESSION['userlevel'] == 1) {
	echo "            <td><a href=\"JavaScript:newPopup('customer_manage.php?id=" . $customers[$i]['id'] . "');\" ><button type=\"button\" class=\"btn btn-primary btn-xs\">Details</button></a></td>";
	echo "          </tr>";
} else {
echo "            <td>-</td>";
echo "            </tr>";
...
// USERLEVELS : Only available for userlevel 2 and above
if ($_SESSION['userlevel'] >= 2) {
echo "<a href=\"JavaScript:newPopup('customer_add.php');\" ><button type\"button\" class=\"btn btn-primary\">Add New Customer</button></a>";
};
?>
      <br /><br />
